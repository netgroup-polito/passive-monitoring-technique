...
if __name__ == "__main__":
    ...
    print ("Extended graph: %d nodes, %d edges" %(n,len(Eedges)))
    ...
    #for edge in extended_graph.edges_iter(): # nx version < 2.0
        if edge[0] in Mnodes or edge[1] in Mnodes:
    ...
    if type(sys.argv[2]) is int:{
        ...
        data.append({"topology":str(sys.argv[1]),"rate":int(sys.argv[2]),"num_clusters":len(clusters),"info_clusters":info_clusters})
    }
    elif type(sys.argv[2]) is str: {
        data.append({"topology":str(sys.argv[1]),"nodes":str(sys.argv[2]),"num_clusters":len(clusters),"info_clusters":info_clusters})
